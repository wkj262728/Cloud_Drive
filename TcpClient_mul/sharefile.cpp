...
void shareFile::updateFriendList(QListWidget *pFriendList)
{
    if(NULL == pFriendList)
    {
        return ;
...
void shareFile::affirmShare()
{
    QString strFileName = opWidget::getInstance().getBook()->m_strSharedFileName;
    QString strFilePath = opWidget::getInstance().getBook()->m_strSharedFilePath;
    ...
    PDU* pdu = mkPDU(strFilePath.toUtf8().size() + userList.count() * 32 + 1); // caMsg中存放文件路径、分享好友名
    pdu -> uiMsgType = ENUM_MSG_TYPE_SHARE_FILE_REQUEST;
    for(int i = 0; i < iUserNum; ++ i)
    {
        strncpy((char*)(pdu -> caMsg) + 32 * i, userList[i].toStdString().c_str(), 32);
    }
    memcpy((char*)(pdu -> caMsg) + 32 * iUserNum, strFilePath.toStdString().c_str(), strFilePath.toUtf8().size());
