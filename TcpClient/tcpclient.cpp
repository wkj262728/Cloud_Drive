...
void TcpClient::startUpdate(const uint &num)
{
    qDebug()<<"start!!";
    QFile file(updateFilePath);
    if(!file.open(QIODevice::ReadOnly))
    {
        MessageTips *mMessageTips = new MessageTips("文件打开失败", this);
        mMessageTips->show();
        file.close();
        return;
    }
    ...
    for(uint i=0; i<=num; i+=4096)
    {
        ...
        if(ret < 0 || ret > 4096)
        {
            MessageTips *mMessageTips = new MessageTips("文件上传失败", this);
            mMessageTips->show();
            break;
        }
        if(ret == 0)break;
        getTcpSocket().write(pBuffer, ret);
        getTcpSocket().waitForBytesWritten();
...
void TcpClient::recvMsg()
{
    ...
    switch (pdu->uiMsgType) {
        ...
        {
            MessageTips *mMessageTips = new MessageTips("文件上传成功", this);
            mMessageTips->show();
