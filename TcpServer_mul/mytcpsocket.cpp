...
void MyTcpSocket::resend(const char *pername, PDU *pdu)
{
    if(pername == NULL || pdu == NULL)return;
    for(auto it:MyTcpServer::getInstance().m_tcpSocketList){
        if(pername == it->getName())
        {
            connect(this, SIGNAL(resending(PDU*))
                    , it, SLOT(onresend(PDU*)));
            emit(resending(pdu));
            disconnect(this, SIGNAL(resending(PDU*))
                       , it, SLOT(onresend(PDU*)));
            break;
        }
    }
...
void MyTcpSocket::onresend(PDU *pdu)
{
    write((char*)pdu, pdu->uiPDULen);
    waitForBytesWritten();
}

void MyTcpSocket::allsend(PDU *pdu)
{
    if(pdu == NULL)return;
    for(auto it:MyTcpServer::getInstance().m_tcpSocketList){
...
void MyTcpSocket::recvMsg()
{
    ...
    case ENUM_MSG_TYPE_REGIST_REQUEST:
    {
        ...
        if(ret)
        {
            ...
            QDir dir;
            dir.mkpath(QString("./file/%1").arg(caName));
        }
        else
        {
...
{
    if(m_strName != "")opDB::getInstance()->handleOff(m_strName.toStdString().c_str());
    emit offline(this);
}
