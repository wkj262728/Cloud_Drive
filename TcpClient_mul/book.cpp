...
void Book::updateFile()
{
    ...
    UpdateFile *newtcpsocket = new UpdateFile(pdu);
    newtcpsocket->moveToThread(thread);
    thread->start();
    connect(this, SIGNAL(startinit())
            , newtcpsocket, SLOT(init()));
    emit(startinit());
    disconnect(this, SIGNAL(startinit())
            , newtcpsocket, SLOT(init()));
}
